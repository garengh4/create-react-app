...
// Require fs to write to tweets.json
const fs = require("fs");
// Requiring a JSON file automatically parses it and returns the data.
let tweetsJSON = require('../data-files/tweets.json');

// Write recent dates to tweets.json
...
module.exports = () => {
  ...
  fs.writeFileSync('server/data-files/tweets.json', JSON.stringify(tweetsJSON, null, 2), { encoding: "utf8" });
};